...
const ClubParticipantsPage: React.FC = () => {
  ...
  // For this demo, we'll simulate the user's club
  const userClub = "OK Linné"; // Simulated user club
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (competitionId && mockCompetitionDetails[competitionId]) {
        ...
        // Generate mock participant data for this competition and filter by user's club
        const allClasses = generateMockParticipantData();
        
        // Filter participants to only include those from user's club
        const filteredClasses = allClasses.map(cls => ({
          ...cls,
          participants: cls.participants.filter(p => p.club === userClub)
        })).filter(cls => cls.participants.length > 0); // Only include classes with participants
        
        setParticipantClasses(filteredClasses);
      }
      setLoading(false);
    }, 500);
  ...
  // Function to generate mock participant data - same as in ParticipantsPage
    ...
    <MobileLayout title="Klubbanmälda" showBackButton>
      <div className="pb-4 space-y-4">
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
          ...
          <p className="text-sm text-primary mt-1">Klubb: {userClub}</p>
        </div>
        ...
        {participantClasses.length > 0 ? (
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 text-center">
            <p className="text-gray-500">Inga klubbmedlemmar anmälda till denna tävling</p>
          </div>
        )}
      </div>
      <Toaster />
